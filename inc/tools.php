...
function vt_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
	...
	if ( $image_src[1] > $width || $image_src[2] > $height ) {
		...
		// if there's an error, just stick to the original image
...
function e404_img_scale($img, $width = 0, $height = 0, $crop = true) {
	// instead of timthumb, using the Victor Teixeira resize function from WP ticket #15311
	$image = vt_resize( '', $img, $width, $height, $crop );
	return $image[url];
